...
export interface RulesetCatalog {
  ...
  origins: Array<{ slug: string; name: string; grantedSkills: string[]; skillChoices: SkillChoiceGroup[] }>;
  classes: Array<{ slug: string; name: string; baseHp: number; hpPerNex: number; baseSan: number; sanPerNex: number; baseEp: number; trainedSkills: number; trainingUpgradeBase: number; grantedSkills: string[]; skillChoices: SkillChoiceGroup[]; paths: Array<{ slug: string; name: string; minNex: number }> }>;
...
export class RulesService {
  ...
  async getCatalog(version: string): Promise<RulesetCatalog> {
    ...
    if (!ruleset) throw new NotFoundException(`Conjunto de regras ${version} não encontrado`);
    ...
    const classSkillGrants = await this.db.select().from(ruleClassSkillGrants).where(sql`true`);
    const classSkillChoices = await this.db.select().from(ruleClassSkillChoices).where(sql`true`);
    const originSkillGrants = await this.db.select().from(ruleOriginSkillGrants).where(sql`true`);
    const originSkillChoices = await this.db.select().from(ruleOriginSkillChoices).where(sql`true`);
    return {
      ...
      origins: origins.map((origin) => ({
        ...
        skillChoices: this.toSkillChoiceGroups(originSkillChoices.filter((choice) => choice.originId === origin.id)),
      })),
      classes: classes.map((ruleClass) => ({
        slug: ruleClass.slug, name: ruleClass.name, baseHp: ruleClass.baseHp, hpPerNex: ruleClass.hpPerNex,
        baseSan: ruleClass.baseSan, sanPerNex: ruleClass.sanPerNex, baseEp: ruleClass.baseEp, trainedSkills: ruleClass.trainedSkills, trainingUpgradeBase: ruleClass.trainingUpgradeBase,
        grantedSkills: classSkillGrants
          .filter((grant) => grant.classId === ruleClass.id)
          .map((grant) => grant.skillSlug),
        skillChoices: this.toSkillChoiceGroups(classSkillChoices.filter((choice) => choice.classId === ruleClass.id)),
  ...
  private toSkillChoiceGroups(
    choices: Array<{ groupSlug: string; skillSlug: string; selectionCount: number }>,
  ): SkillChoiceGroup[] {
