...
export class SpectatorAccessController {
  constructor(
    private readonly spectatorAccessService: SpectatorAccessService,
  ) {}
  ...
  getStatus(
    ...
    return this.spectatorAccessService.getStatus(user.id, campaignId);
  }
  ...
  create(
    ...
    this.assertCampaignId(campaignId);
    return this.spectatorAccessService.create(user.id, campaignId);
  }
  ...
  rotate(
    ...
    this.assertCampaignId(campaignId);
    return this.spectatorAccessService.rotate(user.id, campaignId);
  }
  ...
  async revoke(
    ...
    this.assertCampaignId(campaignId);
    await this.spectatorAccessService.revoke(user.id, campaignId);
  }

  @Get('spectator-access/:token')
  @ApiOperation({
    summary: 'Obtém os dados públicos limitados de uma campanha',
  })
  @ApiOkResponse({
    schema: {
      properties: {
