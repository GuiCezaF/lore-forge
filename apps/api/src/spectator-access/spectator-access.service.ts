...
export interface SpectatorAccessStatusDto {
  isActive: boolean;
}
export interface SpectatorAccessTokenDto {
  token: string;
}
export interface PublicSpectatorCampaignDto {
  name: string;
...
export class SpectatorAccessService {
  ...
  async getStatus(
    userId: string,
    campaignId: string,
  ): Promise<SpectatorAccessStatusDto> {
    await this.assertOwner(userId, campaignId);
    const [access] = await this.db
      .select({ campaignId: campaignSpectatorAccess.campaignId })
  ...
  async create(
    userId: string,
    campaignId: string,
  ): Promise<SpectatorAccessTokenDto> {
    const token = createSpectatorToken();
    try {
      await this.db.transaction(async (tx) => {
        await this.assertOwner(userId, campaignId, tx);
        await tx
          .insert(campaignSpectatorAccess)
          .values({ campaignId, tokenHash: hashSpectatorToken(token) });
      });
    } catch (error) {
      if (this.isUniqueViolation(error))
        throw new ConflictException('Spectator access is already active');
      throw error;
    }
    return { token };
  }
  ...
  async rotate(
    ...
    await this.db.transaction(async (tx) => {
      ...
      const updated = await tx
        .update(campaignSpectatorAccess)
        .set({
          tokenHash: hashSpectatorToken(token),
          updatedAt: new Date().toISOString(),
        })
        .where(eq(campaignSpectatorAccess.campaignId, campaignId))
        .returning({ campaignId: campaignSpectatorAccess.campaignId });
      if (!updated.length)
        throw new ConflictException('Spectator access is not active');
    });
    return { token };
  }
  ...
  async getPublicCampaign(token: string): Promise<PublicSpectatorCampaignDto> {
    ...
    return {
      ...
      hasCoverImage: Boolean(row.coverAssetId),
    };
  }

  async getPublicCover(
    token: string,
  ): Promise<{ body: NodeJS.ReadableStream; mimeType: string }> {
    const row = await this.findPublicAccess(token);
    if (!row.coverAssetId)
      throw new NotFoundException('Spectator access not found');
  ...
  private async findPublicAccess(token: string) {
    ...
    const [row] = await this.db
      ...
      .innerJoin(
        campaigns,
        and(
          eq(campaigns.id, campaignSpectatorAccess.campaignId),
      ...
      .leftJoin(
        mediaAssets,
        and(
          eq(mediaAssets.id, campaigns.coverImageAssetId),
  ...
  private async assertOwner(
    ...
    if (campaign.ownerUserId !== userId)
      throw new ForbiddenException('Only the GM can manage spectator access');
  }
