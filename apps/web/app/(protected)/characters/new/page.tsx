...
type ApiErrorResponse = {
  ...
  message?:
    string | string[] | { field?: AttributeErrorField; message?: string };
};
...
export default function NewPlayerCharacterPage() {
  ...
  const [attributeErrorField, setAttributeErrorField] =
    useState<AttributeErrorField | null>(null);
  ...
  const [draftStatus, setDraftStatus] = useState<"saving" | "saved" | "idle">(
    "idle",
  );
  ...
  function updateNex(nex: number) {
    ...
    const didNormalize = attributes.some(
      (key) => normalizedAttributes[key] !== characterAttributes[key],
    );
    setDerived(null);
    ...
    if (
      !getAvailablePaths(selectedClass?.paths ?? [], nex).some(
        (path) => path.slug === values.path,
      )
    ) {
      update("path", "");
    }
  ...
  function togglePower(power: CharacterPower) {
    setPowers((current) =>
      current.some((selection) => selection.name === power.slug)
  ...
  function updatePowerRank(name: string, rank: number) {
    setPowers((current) =>
      current.map((selection) =>
        selection.name === name ? { ...selection, rank } : selection,
      ),
    );
  }
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const characterClass = rules?.classes.find(
      (item) => item.slug === values.characterClass,
    );
    const granted = getGrantedSkillNames(origin, characterClass);
    const knownSkills = new Set(
      (rules?.skills ?? []).map((skill) => skill.slug),
    );
    const availablePowerNames = new Set(
      getAvailablePowers(
        rules?.powers ?? [],
        values.nex,
    ...
    setSkills((current) => {
      ...
      granted.forEach((name) =>
        merged.set(name, merged.get(name) ?? { name, degree: "trained" }),
      );
      const next = [...merged.values()].map((skill) => ({
        ...skill,
        degree:
          skill.degree === "expert" && values.nex < 70
            ? "trained"
    ...
    const timer = window.setTimeout(() => {
      ...
      void (async () => {
        ...
        const response = draftId
          ? await apiFetch(`${apiUrl}/characters/${draftId}/draft`, {
              method: "PATCH",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify(body),
            })
          : await apiFetch(`${apiUrl}/characters/drafts`, {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify(body),
            });
  ...
  async function submit(event: FormEvent) {
    ...
    const response = draftId
      ? await apiFetch(`${apiUrl}/characters/${draftId}`, {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          body,
        })
      : await apiFetch(`${apiUrl}/characters`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body,
        });
  ...
  const selectedOrigin = rules?.origins.find(
    (item) => item.slug === values.origin,
  );
  ...
  const trainingUpgradeLimit = getTrainingUpgradeLimit(
    selectedClass,
    values.intellect,
  );
  ...
  const choiceSkillNames = new Set(
    choiceGroups.flatMap((group) => group.skills),
  );
  ...
  const freeSkillNames = (rules?.skills ?? [])
    ...
    .filter(
      (name) =>
        !grantedSkillNames.includes(name) && !choiceSkillNames.has(name),
    );
  const selectedFreeCount = skills.filter(
    (skill) =>
      !grantedSkillNames.includes(skill.name) &&
      !choiceSkillNames.has(skill.name),
  ).length;
  const freeSkillCount = selectedClass
    ? selectedClass.trainedSkills + Math.max(0, values.intellect)
    : 0;
  ...
  return (
    ...
    >
        ...
        {steps.map((label, index) => (
          <li key={label}>
            <button
              ...
              className={`w-full rounded-lg border p-3 text-left text-xs ${step === index ? "border-red-700 bg-red-950/30 text-red-200" : "border-zinc-800 text-zinc-500"}`}
            >
              {index + 1}. {label}
            </button>
          </li>
      ...
      <section className="rounded-xl border border-zinc-800 bg-zinc-950 p-6">
        {step === 0 && (
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <Field
              ...
              value={values.sheetLabel}
              onChange={(value) => update("sheetLabel", value)}
            />
            <Field
              label={t("concept")}
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <label className="grid gap-2 text-sm text-zinc-300">
              ...
              <select
                ...
                onChange={(event) => {
                  update("characterClass", event.target.value);
                  update("path", "");
                }}
                className="rounded-md border border-zinc-700 bg-zinc-900 p-2"
              >
            ...
            <label className="grid gap-2 text-sm text-zinc-300">
              ...
              <select
                value={values.nex}
                onChange={(event) => updateNex(Number(event.target.value))}
                className="rounded-md border border-zinc-700 bg-zinc-900 p-2"
              >
                {nexOptions.map((nex) => (
                  <option key={nex} value={nex}>
                    {nex}%
                  </option>
                ))}
              </select>
        ...
        {step === 2 && (
          <div className="grid gap-8">
            <div>
              ...
              {attributeError && (
                <p
                  className="mb-4 rounded-md border border-red-900 bg-red-950/30 p-3 text-sm text-red-200"
                  role="alert"
                >
                  {attributeError}
                </p>
              ...
              <div className="grid gap-4 sm:grid-cols-5">
                {attributes.map((key) => (
                  <label
                    key={key}
                    className={`grid gap-2 rounded-lg border p-3 text-sm text-zinc-300 ${attributeErrorField === key ? "border-red-700" : "border-zinc-800"}`}
                  >
                    {t(`attribute.${key}`)}
                    <input
                      type="number"
                      ...
                      aria-invalid={attributeErrorField === key || undefined}
            ...
            <SkillAndPowerSelection
              skills={skills}
              powers={powers}
              grantedSkillNames={grantedSkillNames}
              choiceGroups={choiceGroups}
              freeSkillNames={freeSkillNames}
              selectedSkillNames={selectedSkillNames}
              selectedFreeCount={selectedFreeCount}
              freeSkillCount={freeSkillCount}
              requiredSkillCount={requiredSkillCount}
              trainingUpgradeCount={trainingUpgradeCount}
              trainingUpgradeLimit={trainingUpgradeLimit}
              availablePowers={availablePowers}
              powerSelectionLimit={powerSelectionLimit}
              skillNames={Object.fromEntries(
                (rules?.skills ?? []).map((skill) => [skill.slug, skill.name]),
              )}
              nex={values.nex}
              onToggleSkill={toggleSkill}
              onUpdateSkillDegree={updateSkillDegree}
              onTogglePower={togglePower}
              onUpdatePowerRank={updatePowerRank}
              t={t}
            />
          </div>
        )}
        {step === 3 && (
          <Review
            values={values}
            preview={preview}
            origin={selectedOrigin?.name ?? t("notSelected")}
            characterClass={selectedClass?.name ?? t("noClass")}
            path={selectedPath?.name ?? t("notSelected")}
            derived={derived}
            attributes={characterAttributes}
            attributeLabels={
              Object.fromEntries(
                attributes.map((key) => [key, t(`attribute.${key}`)]),
              ) as Record<keyof CharacterAttributes, string>
            }
            skills={skills}
            powers={powers}
            skillNames={Object.fromEntries(
            ...
            powerNames={Object.fromEntries(
              (rules?.powers ?? []).map((power) => [power.slug, power.name]),
            )}
            t={t}
          />
        )}
      ...
      {error && (
        <p className="rounded-md border border-red-900 bg-red-950/30 p-3 text-sm text-red-200">
          {error}
        </p>
      )}
      <footer className="flex justify-between">
        <button
          ...
          disabled={step === 0}
          className="rounded-md px-4 py-2 text-sm text-zinc-400 disabled:opacity-30"
        >
          {t("back")}
        </button>
        {step < steps.length - 1 ? (
...
function SkillAndPowerSelection({
  ...
  freeSkillCount,
  requiredSkillCount,
  trainingUpgradeCount,
  trainingUpgradeLimit,
  availablePowers,
  powerSelectionLimit,
  skillNames,
  nex,
  ...
  trainingUpgradeCount: number;
  trainingUpgradeLimit: number;
  availablePowers: CharacterPower[];
  powerSelectionLimit: number;
  ...
  const degreeFor = (name: string) =>
    skills.find((skill) => skill.name === name)?.degree ?? "trained";
  return (
    <div className="grid gap-6 border-t border-zinc-800 pt-6 text-sm text-zinc-300">
      <section className="grid gap-3">
        <div>
          <h2 className="font-semibold text-zinc-100">{t("skills")}</h2>
          <p className="text-zinc-500">
            {t("skillsHelp", {
              selected: skills.length,
              required: requiredSkillCount,
            })}
          </p>
        </div>
        {grantedSkillNames.length > 0 && (
          <div className="grid gap-2">
            ...
            {grantedSkillNames.map((name) => (
              <SkillRow
                key={name}
                name={name}
                label={skillNames[name] ?? name}
                checked
                degree={degreeFor(name)}
                disabled
                nex={nex}
                onToggle={() => onToggleSkill(name, true)}
                onDegreeChange={(degree) => onUpdateSkillDegree(name, degree)}
                t={t}
              />
            ))}
          </div>
        ...
        {choiceGroups.map((group) => {
          const count = group.skills.filter(
            (name) =>
              selectedSkillNames.has(name) && !grantedSkillNames.includes(name),
          ).length;
          return (
            <div key={group.slug} className="grid gap-2">
              <p className="text-xs font-semibold uppercase tracking-widest text-zinc-500">
                {t("skillChoiceGroup", {
                  group: group.slug,
                  selected: count,
                  required: group.selectionCount,
                })}
              </p>
              {group.skills.map((name) => (
                <SkillRow
                  ...
                  degree={degreeFor(name)}
                  disabled={
                    !selectedSkillNames.has(name) &&
                    count >= group.selectionCount
                  }
                  nex={nex}
                  onToggle={() => onToggleSkill(name, false)}
                  onDegreeChange={(degree) => onUpdateSkillDegree(name, degree)}
                  t={t}
                />
              ))}
        ...
        {freeSkillNames.length > 0 && (
          <div className="grid gap-2">
            <p className="text-xs font-semibold uppercase tracking-widest text-zinc-500">
              {t("freeSkills", {
                selected: selectedFreeCount,
                required: freeSkillCount,
              })}
            </p>
            {freeSkillNames.map((name) => (
              <SkillRow
                key={name}
                name={name}
                label={skillNames[name] ?? name}
                ...
                disabled={
                  !selectedSkillNames.has(name) &&
                  selectedFreeCount >= freeSkillCount
                }
                nex={nex}
                onToggle={() => onToggleSkill(name, false)}
                onDegreeChange={(degree) => onUpdateSkillDegree(name, degree)}
        ...
        >
          {t("trainingUpgrades", {
            used: trainingUpgradeCount,
            limit: trainingUpgradeLimit,
          })}
        </p>
      </section>
      <section className="grid gap-3">
        <div>
          <h2 className="font-semibold text-zinc-100">{t("powers")}</h2>
          <p className="text-zinc-500">
            {t("powersHelp", {
              selected: powers.length,
              limit: powerSelectionLimit,
            })}
          </p>
        </div>
        {powerSelectionLimit === 0 ? (
          <p className="text-zinc-500">{t("noPowerSlots")}</p>
        ) : availablePowers.length === 0 ? (
          <p className="text-zinc-500">{t("noPowers")}</p>
        ) : (
          availablePowers.map((power) => {
            const selection = powers.find((item) => item.name === power.slug);
            const isDisabled =
              !selection && powers.length >= powerSelectionLimit;
            return (
              <label
                key={power.slug}
              ...
              >
                <input
                  ...
                  disabled={isDisabled}
                  onChange={() => onTogglePower(power)}
                />
                <span className="flex-1">{power.name}</span>
                {selection && power.maxRank > 1 && (
                  <select
                    aria-label={t("powerRank", { power: power.name })}
                    value={selection.rank}
                    onChange={(event) =>
                      onUpdatePowerRank(power.slug, Number(event.target.value))
                    }
                    className="rounded bg-zinc-900 p-1"
                  >
                    {Array.from(
                      { length: power.maxRank },
                      (_, index) => index + 1,
                    ).map((rank) => (
...
function SkillRow({
  ...
  label: string;
  checked: boolean;
  degree: CharacterSkill["degree"];
  disabled: boolean;
  nex: number;
  onToggle: () => void;
  onDegreeChange: (degree: CharacterSkill["degree"]) => void;
  t: ReturnType<typeof useTranslations>;
}) {
  return (
    <label className="flex flex-wrap items-center gap-3 rounded border border-zinc-800 p-3">
      <input
        type="checkbox"
        checked={checked}
        disabled={disabled}
      ...
      {checked && (
        <select
          aria-label={t("skillDegree", { skill: label })}
          value={degree}
          onChange={(event) =>
            onDegreeChange(event.target.value as CharacterSkill["degree"])
          }
          className="rounded bg-zinc-900 p-1"
        >
          <option value="trained">{t("degree.trained")}</option>
          <option value="veteran" disabled={nex < 35}>
            {t("degree.veteran")}
          </option>
          <option value="expert" disabled={nex < 70}>
            {t("degree.expert")}
          </option>
        </select>
      )}
...
function Review({
  ...
  return (
    <div className="grid gap-6 text-sm text-zinc-300">
      <p className="text-zinc-400">
        {derived ? t("derivedHelp") : t("calculatingDerived")}
      </p>
      <div className="grid gap-4 sm:grid-cols-[auto_1fr]">
        ...
        <dl className="grid gap-3 sm:grid-cols-2">
          ...
          <ReviewValue
            label={t("privateLabel")}
            value={values.sheetLabel || t("notProvided")}
          />
          <ReviewValue
            label={t("concept")}
            value={values.concept || t("notProvided")}
          />
        </dl>
      </div>
      <ReviewGroup title={t("originClass")}>
        ...
        <ReviewValue
          label={t("nex")}
          value={t("nexValue", { value: values.nex })}
        />
      </ReviewGroup>
      <ReviewGroup title={t("attributes")}>
        {attributes.map((key) => (
          <ReviewValue
            ...
            value={String(characterAttributes[key])}
          />
        ))}
      </ReviewGroup>
      <ReviewGroup title={t("skills")}>
        {skills.map((skill) => (
          <ReviewValue
            key={skill.name}
            label={skillNames[skill.name] ?? skill.name}
            value={t(`degree.${skill.degree}`)}
          />
        ))}
      </ReviewGroup>
      <ReviewGroup title={t("powers")}>
        {powers.length > 0 ? (
          powers.map((power) => (
            <ReviewValue
              key={power.name}
              label={powerNames[power.name] ?? power.name}
              value={t("rank", { rank: power.rank })}
            />
          ))
        ) : (
          <ReviewValue label={t("powers")} value={t("noPowersSelected")} />
        )}
      </ReviewGroup>
      <ReviewGroup title={t("derivedValues")}>
        {derived ? (
          derivedLabels.map((key) => (
            <ReviewValue
              key={key}
              label={t(`derived.${key}`)}
              value={
                derived[key] === null ? t("notAvailable") : String(derived[key])
              }
            />
          ))
        ) : (
          <ReviewValue
            label={t("derivedValues")}
            value={t("calculatingDerived")}
          />
        )}
      </ReviewGroup>
    </div>
...
function sameSkills(
  ...
  return (
    ...
    current.every(
      (skill, index) =>
        skill.name === next[index]?.name &&
        skill.degree === next[index]?.degree,
    )
  );
}
