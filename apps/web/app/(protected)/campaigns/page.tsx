        ...
        <section className="rounded-3xl border border-zinc-900 bg-zinc-950/80">
          ...
          {selectedCampaign ? (
            <div className="space-y-5 p-5">
              ...
              {isManager ? <section className="rounded-xl border border-zinc-800 p-4"><div className="flex items-center justify-between gap-3"><div><p className="text-sm font-medium text-zinc-100">Pending invitations</p><p className="text-xs text-zinc-500">Only GMs can cancel an unanswered invitation.</p></div></div>{managedInvitations.length ? <ul className="mt-3 space-y-2">{managedInvitations.map(invitation => <li key={invitation.id} className="flex items-center justify-between gap-3 rounded border border-zinc-800 p-2 text-sm"><span className="text-zinc-300">{invitation.invitedUserName} <span className="text-zinc-500">({invitation.invitedUserShortCode})</span></span><button type="button" onClick={() => void cancelInvitation(invitation.id)} className="text-xs text-red-300">Cancel</button></li>)}</ul> : <p className="mt-3 text-xs text-zinc-500">No pending invitations.</p>}</section> : null}

              <section className="rounded-xl border border-zinc-800 p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-sm font-medium text-zinc-100">Campaign characters</p><p className="text-xs text-zinc-500">State and inventory belong to this campaign only.</p></div>{isManager ? <Link href="/characters/npc/new" className="rounded border border-red-900 px-3 py-2 text-xs text-red-200">Create NPC</Link> : null}</div><div className="mt-3 grid gap-2 sm:grid-cols-2">{campaignPlayers.map(sheet => <Link key={sheet.id} href={`/characters/${sheet.id}`} className="rounded border border-zinc-800 p-2 text-sm text-zinc-300">PC · {sheet.name}</Link>)}{campaignNpcs.map(sheet => <Link key={sheet.id} href={`/characters/${sheet.id}`} className="rounded border border-zinc-800 p-2 text-sm text-zinc-300">NPC · {sheet.name}</Link>)}{!campaignPlayers.length && !campaignNpcs.length ? <p className="text-xs text-zinc-500">No character sheets are attached yet.</p> : null}</div></section>
