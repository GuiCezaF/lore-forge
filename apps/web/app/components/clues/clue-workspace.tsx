...
function toDraft(clue: CampaignClue): ClueDraft {
  return {
    gmLabel: clue.gmLabel,
    title: clue.title ?? "",
    privateNotes: clue.privateNotes ?? "",
    style: clue.style,
...
export function ClueWorkspace({ campaignId }: ClueWorkspaceProps) {
    ...
    async function loadClues() {
      ...
      try {
        const response = await apiFetch(
          `${apiUrl}/campaigns/${campaignId}/clues`,
        );
        if (!response.ok)
          throw new Error("Não foi possível carregar as pistas.");
        ...
        setError(
          loadError instanceof Error
            ? loadError.message
            : "Não foi possível carregar as pistas.",
        );
      } finally {
        setIsLoading(false);
      }
  ...
  async function selectClue(clueId: string) {
    ...
    setError("");
    const response = await apiFetch(
      `${apiUrl}/campaigns/${campaignId}/clues/${clueId}`,
    );
    if (!response.ok) {
      setError("Não foi possível carregar esta pista.");
      return;
    }
  ...
  async function saveClue(event: FormEvent<HTMLFormElement>) {
    ...
    const url = isUpdating
      ? `${apiUrl}/campaigns/${campaignId}/clues/${selectedClue.id}`
      : `${apiUrl}/campaigns/${campaignId}/clues`;
    try {
      ...
      if (!response.ok)
        throw new Error(
          "Não foi possível salvar a pista. Verifique os campos e tente novamente.",
        );
      const clue = (await response.json()) as CampaignClue;
      const nextDraft = toDraft(clue);
      setSelectedClue(clue);
      ...
      setSavedDraft(nextDraft);
      setClues((current) =>
        [clue, ...current.filter((item) => item.id !== clue.id)].sort(
      ...
      setError(
        saveError instanceof Error
          ? saveError.message
          : "Não foi possível salvar a pista.",
      );
    } finally {
      setIsSaving(false);
    }
  ...
  return (
    <div className="flex flex-1 flex-col gap-6 py-4">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-zinc-500">
            Crie documentos privados e visualize a apresentação que será usada
            pelos jogadores.
          </p>
        </div>
        <button
      ...
      {error ? (
        <p
          role="alert"
          className="rounded-lg border border-red-900/70 bg-red-950/30 p-3 text-sm text-red-200"
        >
          {error}
        </p>
      ) : null}
      {!isComposing ? (
        <section className="rounded-3xl border border-zinc-900 bg-zinc-950/80">
          <div className="border-b border-zinc-900 px-5 py-4">
            <h2 className="text-xl font-medium text-zinc-100">Coleção</h2>
          </div>
          <div className="p-5">
            {isLoading ? (
              <p className="text-sm text-zinc-500">Carregando pistas…</p>
            ) : clues.length === 0 ? (
              <p className="text-sm text-zinc-500">
                Nenhuma pista criada ainda.
              </p>
            ) : (
              <ul className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
                {clues.map((clue) => (
                  <li key={clue.id}>
                    <button
                      type="button"
                      onClick={() => void selectClue(clue.id)}
                      className="w-full rounded-2xl border border-zinc-800 bg-zinc-950 p-4 text-left transition hover:border-red-900/70"
                    >
                      <div className="flex items-center justify-between gap-3">
                        <span className="text-xs uppercase tracking-[0.16em] text-red-300">
                          {clue.gmLabel}
                        </span>
                        <span className="text-[10px] uppercase text-zinc-500">
                          {styleLabels[clue.style]}
                        </span>
                      </div>
                      <h3 className="mt-3 font-serif text-lg text-zinc-100">
                        {clue.title || "Sem título"}
                      </h3>
                      <p className="mt-2 text-xs text-zinc-500">
                        Atualizada em{" "}
                        {new Date(clue.updatedAt).toLocaleDateString("pt-BR")}
                      </p>
                    </button>
                  </li>
                ))}
        ...
        <form onSubmit={saveClue} className="space-y-5">
          <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
            <section
              className={`space-y-4 ${mobilePanel === "preview" ? "hidden lg:block" : "block"}`}
            >
              <div className="rounded-2xl border border-zinc-800 bg-zinc-950 p-5 space-y-4">
                ...
                <label className="block space-y-1">
                  <span className="text-xs text-zinc-400">Label privada</span>
                  <input
                    required
                    minLength={1}
                    maxLength={120}
                ...
                <label className="block space-y-1">
                  <span className="text-xs text-zinc-400">Título público</span>
                  <input
                    maxLength={200}
                    value={draft.title}
                    onChange={(event) =>
                ...
                <label className="block space-y-1">
                  ...
                  <select
                    ...
                    onChange={(event) =>
                      setDraft((current) => ({
                        ...current,
                        style: event.target.value as ClueStyle,
                      }))
                    }
                    className="w-full rounded border border-zinc-700 bg-zinc-900 px-3 py-2 text-sm text-zinc-100"
                  >
                    {clueStyles.map((style) => (
                ...
                <label className="block space-y-1">
                  <span className="text-xs text-zinc-400">Notas privadas</span>
                  <textarea
                    maxLength={10000}
                    ...
                    className="min-h-28 w-full rounded border border-zinc-700 bg-zinc-900 px-3 py-2 text-sm text-zinc-100"
                  />
                </label>
              </div>
              <ClueEditor
                key={selectedClue?.id ?? "new"}
                content={draft.content}
                onChange={(content) =>
            ...
            <section
              className={`${mobilePanel === "edit" ? "hidden lg:block" : "block"}`}
            >
          ...
          <div className="flex items-center justify-between gap-3 border-t border-zinc-800 pt-4">
            ...
            >
              ...
              {isSaving ? "Salvando…" : "Salvar pista"}
            </button>
          </div>
          <div className="fixed inset-x-0 bottom-0 z-40 flex border-t border-zinc-700 bg-zinc-950 p-2 lg:hidden">
            <button
              type="button"
              aria-pressed={mobilePanel === "edit"}
              onClick={() => setMobilePanel("edit")}
              className="flex-1 rounded px-3 py-2 text-sm text-zinc-200 aria-pressed:bg-zinc-800"
            >
              Editar
            </button>
            <button
