...
const origin: CharacterOrigin = {
  ...
  skillChoices: [{ slug: "origem", selectionCount: 1, skills: ["atualidades", "investigacao"] }],
};
...
const characterClass: CharacterClass = {
  ...
  skillChoices: [{ slug: "classe", selectionCount: 2, skills: ["crime", "tecnologia", "pilotagem"] }],
};

describe("character creation skill helpers", () => {
  it("counts granted, choice, class and Intellect skill slots", () => {
    expect(getGrantedSkillNames(origin, characterClass)).toEqual(["ciencia", "investigacao"]);
  ...
  it("counts veteran and expert upgrades against the class limit", () => {
    ...
    expect(getTrainingUpgradeCount([
  ...
  it("only exposes powers available to the selected class and NEX", () => {
    expect(getAvailablePowers([
      { slug: "general", name: "Geral", minNex: 5, maxRank: 1, requiredClassSlug: null },
      { slug: "specialist", name: "Especialista", minNex: 15, maxRank: 2, requiredClassSlug: "especialista" },
      { slug: "combatant", name: "Combatente", minNex: 5, maxRank: 1, requiredClassSlug: "combatente" },
    ], 15, "especialista").map((power) => power.slug)).toEqual(["general", "specialist"]);
  });
